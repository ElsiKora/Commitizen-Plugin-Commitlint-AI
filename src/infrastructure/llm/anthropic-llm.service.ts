...
import { MIN_RULE_LENGTH, OPENAI_MAX_TOKENS, OPENAI_TEMPERATURE, RULE_CONFIG_LENGTH, RULE_VALUE_INDEX, VALIDATION_LEVEL_DISABLED, VALIDATION_LEVEL_ERROR } from "../../domain/constant/numeric.constant.js";
...
export class AnthropicLlmService implements ILlmService {
	...
	async generateCommitMessage(context: ILlmPromptContext, configuration: LLMConfiguration): Promise<CommitMessage> {
		...
		const prompt: string = this.buildPrompt(context);
		...
		// Type guard ensures content is TextBlock here
		return this.parseCommitMessage(content.text);
	}
	 ...
	 */
	private buildPrompt(context: ILlmPromptContext): string {
		let prompt: string = "";
		...
		if (isFixing) {
			prompt = "Fix the following commit message to comply with validation rules. Maintain the original meaning and content while fixing only the format issues.\n\n";
		} else {
			prompt = "Generate a conventional commit message based on the following context and rules.\n\n";
		}
		...
		// Only include changes if we're generating (not fixing)
		if (!isFixing) {
			prompt += "\n\nChanges:\n";
		...
		prompt += "\n\nRespond ONLY with the JSON object, no additional text, no markdown code blocks, just the raw JSON.";
	...
	private parseCommitMessage(content: string): CommitMessage {
		try {
			...
			// Remove markdown code blocks - fixed regex
