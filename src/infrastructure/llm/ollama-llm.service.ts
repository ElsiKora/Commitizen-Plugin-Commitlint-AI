...
const DEFAULT_NUM_PREDICT: number = 2048;
const DEFAULT_TEMPERATURE: number = 0.7;
const DEFAULT_PORT: string = "11434";
...
export class OllamaLlmService implements ILlmService {
	...
	async generateCommitMessage(context: ILlmPromptContext, configuration: LLMConfiguration): Promise<CommitMessage> {
		...
		try {
			...
			const responseData: { response?: string } = (await response.json()) as { response?: string };
			const content: string | undefined = responseData.response;
